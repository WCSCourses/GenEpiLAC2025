...
CostaRica_assembly.stats <- read_sheet("https://docs.google.com/spreadsheets/d/1nd69T_NjmFiJnRwIIMx1pRnRgq0uE_fp4CZ3jDB2S_Y/edit?usp=sharing", sheet="Student_Results")
...
```{r}
CostaRica_assembly.stats %>%
  arrange((`Genome fraction (% of reference)`)) %>%
  mutate(method_full=factor(`Assembly Type (short/long/hybrid)`, levels=unique(`Assembly Type (short/long/hybrid)`))) %>%
  filter(!is.na(N50))  %>%
  ggplot(aes(y=method_full, x=`Genome fraction (% of reference)`, color=factor(`Number of long reads (if used)`))) +
  #ggplot(aes(y=method_full, x=`N50`)) +
  geom_quasirandom(width=0.4, alpha=0.75, groupOnX=F) +
  theme_bw() + theme(legend.position = 'bottom') + 
  theme.text.size + 
  facet_grid(`Assembler (Unicycler/Dragonflye)`~.) +
  labs(y="Assembly Type", x="N50", title="% ref genome recovered for each Assembly Type", color="# Long Reads")

ggsave(paste0("Assembly.Type.+.readinput.by.perc-genome-rec.png"), units='mm', width=120, height=90, device='png', dpi=300)
```
...
ggsave(paste0("Student.by.Contig.count.png"), units='mm', width=110, height=140, device='png', dpi=900)
 ...
 CostaRica_assembly.stats %>%
  arrange((`Genome fraction (% of reference)`)) %>%
  mutate(student_method=factor(`student_method`, levels=unique(`student_method`))) %>%
  ggplot(aes(y=`student_method`, x=`Genome fraction (% of reference)`, color=`Student Name`)) +
  #geom_quasirandom(width=0.4, alpha=0.75, groupOnX=F) +
  geom_quasirandom() +
  #geom_barh(width=0.5, stat='identity') + 
  theme_bw() + theme(legend.position = 'none') + 
  theme.text.size + 
  geom_text(aes((`Genome fraction (% of reference)`+10), `student_method`,label=`Student Name`), size=2.5) +
  labs(y="Student", x="# Contigs", title="Genome fraction (% of reference)") +
  #coord_cartesian(xlim=c(1,500)) #+ scale_x_log10() +
NULL
ggsave(paste0("Student.by.perc-genome-recovered.png"), units='mm', width=110, height=140, device='png', dpi=900)
```
